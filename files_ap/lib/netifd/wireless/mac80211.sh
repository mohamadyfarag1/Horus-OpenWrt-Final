...
get_freq() {
	...
	# Horus Superchannel frequency resolution
	if [ "$channel" -ge 2000 ] 2>/dev/null; then
		echo "$channel"
		return 0
	fi
	if [ "$band" = "2g" ]; then
		if [ "$channel" -ge 237 ] && [ "$channel" -le 255 ]; then
			echo "$(( 2407 + (channel - 256) * 5 ))"
			return 0
		elif [ "$channel" -eq 256 ]; then
			echo "2407"
			return 0
		elif [ "$channel" -ge 1 ] && [ "$channel" -le 13 ]; then
			echo "$(( 2407 + channel * 5 ))"
			return 0
		elif [ "$channel" -eq 14 ]; then
			echo "2484"
			return 0
		elif [ "$channel" -eq 74 ]; then
			echo "2477"
			return 0
		elif [ "$channel" -eq 75 ]; then
			echo "2482"
			return 0
		elif [ "$channel" -ge 76 ] && [ "$channel" -le 80 ]; then
			echo "$(( 2487 + (channel - 76) * 5 ))"
			return 0
		elif [ "$channel" -ge 15 ] && [ "$channel" -le 59 ]; then
			echo "$(( 2437 + channel * 5 ))"
			return 0
		fi
	fi
