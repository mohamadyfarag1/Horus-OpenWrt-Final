...
function client_ratelimit(uspot, mac) {
	...
	if (client.radius?.reply) {
		...
		// Horus/MikroTik: 'Mikrotik-Rate-Limit' = "rxrate[/txrate] [bursts...]"
		// where rx = client upload, tx = client download; units bare=bps, k/M/G.
		// SAS is registered as a MikroTik NAS, so this takes priority.
		let mtrl = reply['Mikrotik-Rate-Limit'];
		if (mtrl) {
			let parse_rate = function(s) {
				let m = match(s, /^([0-9]+)([kKmMgG]?)/);
				if (!m)
					return 0;
				let n = int(m[1]);
				switch (lc(m[2])) {
				...
				case 'g': return n * 1000000000;
				default:  return n;
				}
			};
			let parts = split(trim(mtrl), /[ \t]+/);
			let mainr = split(parts[0], '/');
			maxup = parse_rate(mainr[0]);					// rx = client upload
			maxdown = (length(mainr) > 1) ? parse_rate(mainr[1]) : maxup;	// tx = client download
		}

		// fallback to known attributes - WISPr: bps, ChiliSpot: kbps
		if (!(+maxup || +maxdown)) {
...
function client_quotalimit(uspot, mac) {
	...
	// Horus/MikroTik quota attributes (bytes). SAS commonly sends Total-Limit.
	if (!maxtotal && reply['Mikrotik-Total-Limit'])
		maxtotal = (+reply['Mikrotik-Total-Limit'] + (+reply['Mikrotik-Total-Limit-Gigawords'] << 32)) || 0;
	if (!maxup && reply['Mikrotik-Xmit-Limit'])
		maxup = (+reply['Mikrotik-Xmit-Limit'] + (+reply['Mikrotik-Xmit-Limit-Gigawords'] << 32)) || 0;
	if (!maxdown && reply['Mikrotik-Recv-Limit'])
		maxdown = (+reply['Mikrotik-Recv-Limit'] + (+reply['Mikrotik-Recv-Limit-Gigawords'] << 32)) || 0;
