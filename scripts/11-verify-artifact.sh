...
if [ -z "$ROOTFS_HORUS_BOUNDS" ] || [ ! -f "$ROOTFS_HORUS_BOUNDS" ]; then
    fail_assertion "horus-5g-bounds missing from rootfs" \
        ...
        "This file is GENERATED by write_5g_bounds() in scripts/gen_package_patches.py into openwrt/files/lib/netifd/. Its absence means that generator did not run - check that scripts/06-compile.sh invokes gen_package_patches.py before the rootfs is packaged."
fi
...
if [ -n "$PLAN_MIN" ] && [ -n "$PLAN_MAX" ]; then
    if [ "$PLAN_MIN" != "$BOUNDS_MIN" ] || [ "$PLAN_MAX" != "$BOUNDS_MAX" ]; then
        fail_assertion "horus-5g-bounds does not match the compiled channel plan" \
            ...
            "The bounds file is generated from CHANS by write_5g_bounds(), so a mismatch means a stale file survived from an earlier build. Remove openwrt/files/lib/netifd/horus-5g-bounds and re-run scripts/gen_package_patches.py."
    fi
