...
_BLOCKS_5G = [
    (221, 4920, 32), (16, 5080, 205),   # 5110 - 6010 -> ch 22..202
]
...
_BLOCKS_2G = [
    ...
    (264, 2512, 1),    # 2512 ONLY (Stop here to prevent AP freezing on QCA4019 PLL lock fail)
]
...
def patch_ath10k(build_dir, pkg_dir):
    ...
    scan_r1 = (
        ...
        "\t}\n"
        "\t/* Add standard channels to active_freqs to ALWAYS scan them */\n"
        "\t{\n"
        "\t\tint std_freqs[] = {2412, 2437, 2462, 2484, 5180, 5200, 5220, 5240, 5745, 5765, 5785, 5805, 5825};\n"
        "\t\tint num_std = sizeof(std_freqs) / sizeof(std_freqs[0]);\n"
        "\t\tint k, j;\n"
        "\t\tfor (k = 0; k < num_std && num_active < 30; k++) {\n"
        "\t\t\tbool dup = false;\n"
        "\t\t\tfor (j = 0; j < num_active; j++) {\n"
        "\t\t\t\tif (active_freqs[j] == std_freqs[k]) { dup = true; break; }\n"
        "\t\t\t}\n"
        "\t\t\tif (!dup) active_freqs[num_active++] = std_freqs[k];\n"
        "\t\t}\n"
        "\t}\n"
        "\tbands = hw->wiphy->bands;\n"
        "\tfor (band = 0; band < NUM_NL80211_BANDS; band++) {\n"
    ...
    scan_r4 = (
        "\tif (req->n_channels) {\n"
        "\t\targ.n_channels = 0;\n"
        "\t\t/* Pass 1: ALWAYS include standard channels and currently active channels */\n"
        "\t\tfor (i = 0; i < req->n_channels && arg.n_channels < 60; i++) {\n"
        "\t\t\tint f = req->channels[i]->center_freq;\n"
        "\t\t\tbool keep = false;\n"
        "\t\t\tif (f == 2412 || f == 2437 || f == 2462 || f == 2484) keep = true;\n"
        "\t\t\tif (f == 5180 || f == 5200 || f == 5220 || f == 5240) keep = true;\n"
        "\t\t\tif (f == 5745 || f == 5765 || f == 5785 || f == 5805 || f == 5825) keep = true;\n"
        "\t\t\tif (ar->rx_channel && f == ar->rx_channel->center_freq) keep = true;\n"
        "\t\t\tif (keep) {\n"
        "\t\t\t\targ.channels[arg.n_channels++] = f;\n"
        "\t\t\t}\n"
        "\t\t}\n"
        "\t\t/* Pass 2: Round-robin the remaining superchannels */\n"
        "\t\tif (arg.n_channels < req->n_channels) {\n"
        "\t\t\tstatic unsigned int sta_scan_cycle = 0;\n"
        "\t\t\tunsigned int slots = 60 - arg.n_channels;\n"
        "\t\t\tif (slots == 0) slots = 1;\n"
        "\t\t\tunsigned int start_idx = (sta_scan_cycle * slots) % req->n_channels;\n"
        "\t\t\tfor (i = 0; i < req->n_channels && arg.n_channels < 60; i++) {\n"
        "\t\t\t\tint idx = (start_idx + i) % req->n_channels;\n"
        "\t\t\t\tint f = req->channels[idx]->center_freq;\n"
        "\t\t\t\tbool is_std = false;\n"
        "\t\t\t\tif (f == 2412 || f == 2437 || f == 2462 || f == 2484) is_std = true;\n"
        "\t\t\t\tif (f == 5180 || f == 5200 || f == 5220 || f == 5240) is_std = true;\n"
        "\t\t\t\tif (f == 5745 || f == 5765 || f == 5785 || f == 5805 || f == 5825) is_std = true;\n"
        "\t\t\t\tif (ar->rx_channel && f == ar->rx_channel->center_freq) is_std = true;\n"
        "\t\t\t\tif (!is_std) {\n"
        "\t\t\t\t\targ.channels[arg.n_channels++] = f;\n"
        "\t\t\t\t}\n"
        "\t\t\t}\n"
