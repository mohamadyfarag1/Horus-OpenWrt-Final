...
def patch_ath10k(build_dir, pkg_dir):
    ...
    scan_r1 = (
        ...
        "\t/* Identify active operating channels to protect them from rotation */\n"
        "\tlist_for_each_entry(arvif, &ar->arvifs, list) {\n"
        "\t\tif (arvif && arvif->vif && arvif->vif->bss_conf.chandef.chan) {\n"
        "\t\t\tint f = arvif->vif->bss_conf.chandef.chan->center_freq;\n"
        "\t\t\tif (num_active < 8) active_freqs[num_active++] = f;\n"
        "\t\t}\n"
        "\t}\n"
        "\tbands = hw->wiphy->bands;\n"
        "\tfor (band = 0; band < NUM_NL80211_BANDS; band++) {\n"
        ...
        "\t\tunsigned int num_cycles = (horus_rotatable + (slots - 1)) / slots;\n"
