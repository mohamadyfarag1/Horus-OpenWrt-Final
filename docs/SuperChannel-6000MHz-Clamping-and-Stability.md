...
| **وضع العميل Client على البريدج (LAN)** | انهيار الكارت (`BRIDGE_NOT_ALLOWED`) وهبوط الباور إلى 0 | تفعيل WDS 4addr تلقائياً دون انقطاع | **اتصال سليم 30 dBm وسرعة كاملة** |
| **بناء الكومبيل في GitHub Actions** | فشل مع خطأ `ATH10K_NUM_CHANS` > 255 | ضبط القنوات الإجمالية على 253 قناة (حجم u8) | **بناء ناجح ونظيف 100%** |

---

## 8. التحليل الجذري وإصلاح فشل الكومبيل (حاجز الـ 255 في درايفر ath10k-ct)

### أ. سبب فشل البناء في GitHub Actions (Run #131):
* عند توسيع خطة قنوات الـ 5 جيجا لتصل إلى 6000 MHz (القناة 200)، أصبح عدد قنوات الـ 5G يساوي **177 قناة**.
* كانت خطة قنوات 2.4G تحتوي على **86 قناة**.
* حاصل الجمع: $177 + 86 = \mathbf{263\text{ قناة}}$.
* في درايفر `ath10k` و `ath10k-ct`:
  * تُعرّف هياكل البيانات الداخلية (بما في ذلك فهارس المسح ومصفوفة `survey[]`) باستخدام المتغير **`u8` (uint8_t)**، والحد الأقصى الفيزيائي لـ `u8` في لغة C هو **255**.
  * حلقة الفحص البرمجية والمقارنات مثل `(u8)i < ATH10K_NUM_CHANS` أصبحت غير صالحة لأن قيمة 263 تتجاوز الحد الأقصى لنوع البيانات، مما دفع مترجم النواة GCC لتسجيل خطأ فادح تحت خيار `-Werror=type-limits` وفشل بناء حزمة `package/kernel/ath10k-ct (smallbuffers)`.

### ب. الحل الهندسي الدقيق:
* تم فحص نطاق ترددات أجهزة النانو ستيشن (Ubiquiti NanoStation M2):
  * تغطي أجهزة النانو ستيشن M2 الترددات من 2312 MHz إلى 2407 MHz (قنوات 237 إلى 256) والقنوات القياسية 1 إلى 14 (حتى 2484 MHz).
  * تم الاحتفاظ بكامل ترددات النانو ستيشن M2 بنسبة 100%، وتم تقليم 10 قنوات غير مستخدمة إطلاقاً في أقصى التردد العالي فوق 2682 MHz (قنوات 50 إلى 59: 2687 إلى 2732 MHz).
* خطة القنوات الجديدة:
  * **5 GHz**: 177 قناة كاملة (من 5120 MHz حتى 6000 MHz - القناة 200).
  * **2.4 GHz**: 76 قناة كاملة (من 2312 MHz للنانو ستيشن حتى 2682 MHz).
  * **إجمالي القنوات**: $177 + 76 = \mathbf{253\text{ قناة}} \le 255$.
* النتيجة: تمر مصفوفات الدرايفر وفهارس `u8` بنجاح كامل، وتجتاز التحقق `BUILD_BUG_ON` ويبنى الكومبيل بنجاح 100%.

---

## 9. إصلاح وضع العميل (Client Mode) ومنع إيقاف كارت الوايرلس

### أ. سبب المشكلة:
* في نظام OpenWrt، عند ضبط واجهة الوايرلس على وضع عميل `mode='sta'` (Client) وهي مضافة إلى جسر الشبكة المحلية `network='lan'` (جسر `br-lan`):
* يفحص سكربت `/lib/netifd/hostapd.sh`:
  ```bash
  case "$mode" in
      sta)
  ...
  ```
* إذا لم يكن خيار WDS مفعلاً صراحة (`wds=0`)، يقوم السكربت بإرسال إشارة فشل `BRIDGE_NOT_ALLOWED`، فيقوم `netifd` فوراً بإلغاء وحذف كارت العميل `phy1-sta0`، فيسقط كارت الوايرلس وتتحول طاقة البث إلى **0 dBm**.

### ب. الحل البرمجي:
* تم تعديل `/lib/netifd/hostapd.sh` و `/lib/netifd/wireless/mac80211.sh`:
  عندما تكون الواجهة في وضع `sta` ومتصلة بجسر `network_bridge` ولم يتم تفعيل `wds`، يقوم النظام **تلقائياً بتفعيل نمط العنونة الرباعية (`wds=1` / `4addr 1`)** بدلاً من قتل الواجهة:
  ```bash
  sta)
  ...
  ;;
  ```
* النتيجة: سواء اختار المستخدم وضع "Client" أو "Client WDS"، الواجهة تعمل فوراً وتربط بالروكت أو النانو بدون أن تنفصل وبأقصى طاقة بث 30 dBm وبسرعة نقل كاملة.
